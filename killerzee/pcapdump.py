...
class PcapReader:
    ...
    def pcap_next(self):
        '''
        Retrieves the next packet from the capture file.  Returns a tuple of
        (Hdr, packet) where Hdr is a tuple of (timestamp, snaplen, plen) and
        ...
        try:
            ...
            raise EOFError("End of packet capture")
...
class PcapDumper:
    def __init__(self, datalink, savefile, ppi = False):
        ...
        @rtype: None
        '''
        if ppi: from killerzee.pcapdlt import DLT_PPI
        self.ppi = ppi
